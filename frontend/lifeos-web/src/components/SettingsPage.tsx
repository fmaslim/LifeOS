...
import type { ProviderSnapshot } from '../models/integrationProvider'
import { ProviderStatusPanel } from './ProviderStatusPanel'

type PageState = 'ready' | 'loading' | 'empty' | 'error'
interface SettingsPageProps { data: SettingsData; providers: ProviderSnapshot[]; state?: PageState }
...
export function SettingsPage({ data, providers, state = 'ready' }: SettingsPageProps) {
  ...
  return <div className="dashboard settings-page">{header}
    ...
    </div><aside className="settings-side"><ProviderStatusPanel providers={providers} /><DataTransferPanel /><section className="settings-card privacy-card"><div className="settings-card-heading"><div><p className="eyebrow">Privacy & data</p><h2>You stay in control</h2></div></div><div className="privacy-list">{data.privacyItems.map(item => <article key={item.title}><div><h3>{item.title}</h3><p>{item.description}</p></div><button className="text-button" disabled>{item.action}</button></article>)}</div></section></aside></div>
  </div>
}
