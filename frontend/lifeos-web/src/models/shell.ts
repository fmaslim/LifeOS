...
export type RouteName = 'dashboard' | 'inbox' | 'production-health' | 'releases' | 'backups' | 'daily-brief' | 'weekly-review' | 'assistant' | 'approvals' | 'today' | 'activity' | 'calendar' | 'notes' | 'tasks' | 'goals' | 'kpis' | 'projects' | 'planning' | 'habits' | 'routines' | 'learning' | 'contacts' | 'reading' | 'documents' | 'budget' | 'property' | 'content-pipeline' | 'automation-builder' | 'automations' | 'schedules' | 'automation-history' | 'github' | 'agent-control' | 'content' | 'jarvis' | 'dociq' | 'finances' | 'home' | 'health' | 'settings'
