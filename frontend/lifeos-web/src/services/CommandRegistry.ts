...
export function createCommandRegistry(navigation: ShellNavigationItem[]) {
  ...
  registry.register({ id: 'new-routine', label: 'New routine', category: 'Create', keywords: ['routine', 'morning', 'evening', 'checklist', 'recurring'], target: { route: 'routines' }, enabled: true })
  registry.register({ id: 'quick-capture', label: 'Capture to Inbox', category: 'Create', keywords: ['quick', 'idea', 'link', 'reminder'], target: { route: 'inbox', action: 'new' }, enabled: true })
