...
const run = (id: string, automationId: string, automationName: string, status: AutomationRun['status'], hours: number, retryCount = 0, errorDetails?: string): AutomationRun => ({ id, automationId, automationName, trigger: hours > 10 ? 'Schedule' : 'Manual', startedAt: ago(hours), endedAt: ago(hours - .02), durationMs: 72_000, status, outputSummary: status === 'completed' || status === 'retried' ? 'Workflow finished and published its local output.' : 'Workflow stopped before producing output.', errorDetails, retryCount, relatedLinks: [{ label: 'Open automation', href: '#/automations' }] })
const historySeed = [run('run-brief-42','morning-brief','Morning Command Brief','completed',2),run('run-property-18','property-sync','Property Ledger Sync','failed',5,2,'Provider request failed token=private-value https://provider.example/private'),run('run-followup-17','lead-follow-up','Lead Follow-up','retried',9,1),run('run-content-11','content-planning','Content Planning Session','canceled',28)]
