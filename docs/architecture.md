...
`services/NextActionService.ts` composes context-aware "what should I do next" recommendations entirely from other services' already-computed state - `PlanningService.getOverview()` for project/goal next-action, blocked, and critical-path status, `RoutineService`'s today's instances, `KpiService`'s own progress/trend helpers, `WeeklyReviewService`'s proposed priorities, and `DailyBriefService`'s composed notification action - rather than recomputing any of that logic. The deterministic scoring/ranking/evidence derivation lives in `services/NextActionLogic.ts` (mirroring `PlanningLogic.ts`/`BackupLogic.ts`); each candidate source is read in isolation, so one failing dependency drops only that source's candidates (with a warning) instead of ever throwing - the bounded ranked list is always deterministic. Accepting a recommendation that implies a real mutation (raising a task's priority, or creating a follow-up task) always requires explicit approval through `ApprovalService`; a recommendation whose action is only to navigate has nothing to accept. A recommendation sourced from a Weekly Review priority delegates its accept/dismiss entirely to `WeeklyReviewService.requestPriority`/`dismissPriority` rather than re-implementing that approval flow. Dismissed/deferred recommendations are persisted as an evidence-hash-pinned decision (`storageKeys.nextActionDecisions`); a suppressed recommendation only resurfaces once its evidence actually changes or, for a defer, once the defer window elapses.

`services/SmokeCheckService.ts` runs the bounded production smoke suite (frontend availability, `/api/auth/session` routing, the sign-in gate, critical API reachability, and a representative protected read-only flow) after a frontend or backend deployment. It reuses `ReleaseService.current()` for deployed revision/build metadata instead of tracking provenance separately, and publishes failures through the same `ActivityService`/`NotificationService`/`AutomationRunRepository` convention `ProductionHealthService` and `ReleaseService.requestRollback` use. The deterministic pass/fail rules - including detecting the historical `/api/auth/session` SPA-fallback regression (nginx's `try_files` fallback serving `index.html`, HTML, for an API path instead of proxying it) - live in the pure `services/SmokeCheckLogic.ts` evaluators, each probe wraps one in `createDefaultSmokeCheckProbes()` with a deterministic simulated response for the in-app **Smoke Checks** workspace, and one probe throwing never fails the others (`Promise.allSettled`, mirroring `ProductionHealthService`). `scripts/smoke-check.mjs` is a separate, zero-build Node CLI that runs the same rules (duplicated as plain-JS evaluators in `scripts/smokeCheckEvaluators.mjs` so the script has no build-step dependency) as real, read-only GET requests against a live deployment; it is a manual/pipeline tool only - nothing in this repo invokes it automatically against real production URLs.
